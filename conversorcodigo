...
st.title("Conversor de PDF para Excel - Relatórios Escolares")
...
if uploaded_file:
    ...
    with pdfplumber.open(uploaded_file) as pdf:
        for page_num, page in enumerate(pdf.pages):
            ...
            for linha in linhas:
                ...
                if registros:
                    pos_registro = linha.find(registros[0])
                else:
                    pos_registro = len(linha)
    ...
    st.download_button("📥 Baixar Excel", output, file_name="relatorio_convertido.xlsx")
